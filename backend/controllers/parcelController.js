...
exports.unlockLocker = async (req, res) => {
  try {
    const { id } = req.params;
    // Call MQTT service to unlock the locker
    const result = await mqttService.unlockLocker(id);
    res.json({ message: 'Locker unlock command sent', locker_id: id, mqtt_result: result });
  } catch (error) {
